...
if [[ -f $CONFIG_FILE ]]; then
    ...
    root_chat_id=$(sed -n -E 's/^\s*#define ROOT_CHAT_ID\s+([0-9]+)/\1/p' $CONFIG_FILE)
fi

read -p "Enter telegram bot token: " new_bot_token
...
read -p "Enter administrator user id: " new_root_chat_id
...
if ! [[ "$root_chat_id" =~ ^[0-9]+$ ]]; then
    echo -e "$ERRORSTAMP administrator user id is not a number"
    exit 1
fi
